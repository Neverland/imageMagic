...
void function(window, undefined) {
    ...
    WorkSpace = function () {
        ...
        indicator.fn.workInit = function () { //图片上传组件控制器
            var that = this,
            ...
            ({
                ...
                'galleryPost': function () {
                    ...
                    if (tempdata.length === 0) {
                        //alert('请选择要上传的图片！');
                        tips=IM.dailog({name:'uploadTips',title:'提示',html:'<p><b>请选择要上传的图片！</b></p><p>2秒后自动关闭</p>',width:300,height:100},function(){
                    ...
                    var ajax = new IM.tk.ajax(),
                        ...
                        callback = {
                            success: function (a, b) {
                                ...
                                ({
                                    'ok': function () {
                                        dailog.container.innerHTML='<p><b>选中图片已上传完成！</b></p>';
                    ...
                    ajax.request({
                        ...
                        dailog=IM.dailog({name :'uploading',title:'上传',html:'<p><b>正在上传,请耐心等待！</b></p><p><input type="button" id="uploadImageCancel" value="  取消  " /> </p>',width:300,height:100},function(){
        ...
        indicator.fn.removeImageItem = function (elem) {
            ...
            IM.animate(parent, 'left', {
                ...
                callback: function () {
                    ...
                    that.galleryMassage.innerHTML = '图片总数：' + (that.imageListLength);
...
//            } catch (e) {}
//        };
        /*indicator.fn.imageNativeSrc = function(img) {//获得本地上传的图片数据
         ...
         alert('提示：图片' + b.name + '大小超过 ' + IMC.imageLoader.dataSize / 1024000 + 'M 限制，没有导入！');
         }
         });
         };*/
        ...
        indicator.fn.imageNativeSrc = function (img) { //获得本地上传的图片数据
            ...
            IM.tk.each(srcs, function (a, b) {
                if (b.size > IMC.imageLoader.dataSize) {
                    ...
                    /*temp=IM.dailog({name :a,title:'提示',html:'<p align="center">图片' + b.name + '大小超过 ' + IMC.imageLoader.dataSize / 1024000 + 'M 限制，没有导入！</p><p align="center">2秒后自动关闭</p>',width:300,height:100},function(){
                        //var that=this;
                        //this.removeDailog()
                        //setTimeout(function(){temp.removeDailog()},2000);
                    });*/

                    temp || (temp=IM.dailog({name :'imageSizeTip',title:'提示',width:360,height:120}));

                    temp.container.innerHTML+='<p>'+a+'.图片<b>' + b.name + '</b>大小超过 ' + IMC.imageLoader.dataSize / 1024000 + 'M 限制，没有导入！</p>';
        ...
        indicator.fn.imageWebSrc = function (src) {
            ...
            IM.tk.dynamicScriptProxy(temp.url, param, function (b) {
                ...
                //that.setWebSrc.call(that,'data:image/jpeg;base64,'+data.imgdata,data.name)新增
                data.status === 'error' && (alert('网络图片读取失败'),that.initWebLoader());
            });
        };
        ...
        /*indicator.fn.createImageList=function(data,name){
         var that=this;
         that.html+='<dl><dt><img src="'+data+'" title="'+name+'" /><span><input type="checkbox" checked="true" /><input type="button" class="del" value="删" /></span></dt><dd>'+name+'</dd></dl>';
         that.imageListLength++;
        ...
        /*indicator.fn.createImageList = function () {
         var that = this;
         that.html.push('<dl><dt><img id="images' + that.index + '" src="" title="" /><span><input type="checkbox" /><input type="button" class="del" value="删" /></span></dt><dd id="imagesTitle' + that.index + '"></dd></dl>'),
         that.imageListLength++;
         that.index++;
         };*/
        ...
        IM._clip = { //工作区剪裁静态方法
            init: function () {
                ...
                that.defaultValue.call(that); //启用工作区clip预置剪裁框
                that.clientValue(); //启用工作区clip用户自定义剪裁框
	        ...
	        addAreaSize : function(){
		        ...
		        IM.tk.dynamicScriptProxy(url,param,function(b){
			        ...
			        IM.dailog({name:'uploadTips',title:'提示',html:'<p>收藏成功</p><p>2秒后自动关闭</p>',width:300,height:100},function(){
                        var that=this;
        ...
        IM._mark = {
            ...
            uploadImg: function () {
                ...
                if (that.markSrc == null) {
                    //alert('请选择水印');
                    IM.dailog({name:'uploadTips',title:'提示',html:'<p>请选择水印!</p><p>2秒后自动关闭</p>',width:300,height:100},function(){
                        var that=this;
                        setTimeout(function(){that.removeDailog()},2000);
                    });
                    setTimeout(function() {
                ...
                img.onload = function () {
                    ...
                    if (that.xy === false) {
                        //alert('请选择水印位置');
                        IM.dailog({name:'uploadTips',title:'提示',html:'<p>请选择水印位置!</p><p>2秒后自动关闭</p>',width:300,height:100},function(){
                            var that=this;
                            setTimeout(function(){that.removeDailog()},2000);
                        });
                        IM.workSpace.buttonReset(that.button);
